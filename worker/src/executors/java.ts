...
export const executeJava = (
  ...
  return new Promise((resolve) => {
    ...
    const filePath = path.join(workspace, "Main.java");
    ...
    const command =
      `docker run --rm --network=none --memory=256m --cpus=1 -i -v "${workspace}:/workspace" eclipse-temurin:21 sh -c "javac /workspace/Main.java && java -cp /workspace Main < /workspace/input.txt"`;
