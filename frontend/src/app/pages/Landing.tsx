    ...
    <div className="bg-[#0d0d0d] border border-[#2a2a2a] rounded-2xl overflow-hidden shadow-2xl shadow-black/50">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-[#2a2a2a] bg-[#141414]">
        ...
        <span className="ml-3 text-xs font-mono text-[#666] tracking-wide">
          main.
          {line.lang === "cpp" ? "cpp" : line.lang === "java" ? "java" : line.lang}
        </span>
        <span className="ml-auto text-xs font-mono text-[#444] capitalize">{line.lang}</span>
      </div>
      <div className="p-5 font-mono text-sm min-h-[120px] flex flex-col justify-center">
        <div className="flex items-start gap-3">
          ...
          <span className={`${LANG_COLORS[line.lang] ?? "text-green-400"} leading-relaxed`}>
            {typed}
        ...
        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#1e1e1e]">
          ...
          <span className="text-green-400/70 text-xs font-mono">Executed in 142ms</span>
        </div>
      </div>
...
const FEATURES = [
  { icon: Terminal, title: "10+ Languages", desc: "Python, JavaScript, C, C++, Java, Go, PHP, Ruby, C#, Kotlin — all in one place.", color: "text-blue-400", bg: "bg-blue-500/10" },
  { icon: Zap, title: "Instant Execution", desc: "Run code in milliseconds with real-time stdout, stderr, and execution time.", color: "text-primary", bg: "bg-primary/10" },
  { icon: FolderOpen, title: "Project Saving", desc: "Save your work to named projects and pick up right where you left off.", color: "text-purple-400", bg: "bg-purple-500/10" },
  { icon: GitBranch, title: "Version History", desc: "Save snapshots of your code and restore any previous version instantly.", color: "text-green-400", bg: "bg-green-500/10" },
  { icon: Shield, title: "Docker-Isolated", desc: "Every execution runs in a sandboxed Docker container — fully isolated and secure.", color: "text-yellow-400", bg: "bg-yellow-500/10" },
  { icon: Code2, title: "Monaco Editor", desc: "The same editor powering VS Code — syntax highlighting, auto-complete, and more.", color: "text-cyan-400", bg: "bg-cyan-500/10" },
];

const LANGS = ["Python","JavaScript","C","C++","Java","Go","PHP","Ruby","C#","Kotlin"];
...
export default function Landing() {
  ...
  // FIX: Landing never checked auth state, so a returning signed-in user
  // (session restored silently by Clerk from cookies/cache) still saw the
  // marketing page instead of being routed to their dashboard.
  // Mirrors the same redirect pattern already used in Login.tsx.
  useEffect(() => {
  ...
  // FIX: while Clerk is still determining auth state (or about to redirect
  // a signed-in user), show a brief loading state instead of flashing the
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-[#b3b3b3] text-lg leading-relaxed mb-8 max-w-lg">
              A cloud-based multi-language IDE built for learners. Write, run, and save code
              across 10+ languages — no setup, no installs, just code.
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {LANGS.map((l) => (
                <span key={l} className="px-2.5 py-1 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg text-xs font-mono text-[#b3b3b3]">
                  {l}
                </span>
              ))}
            </div>
          ...
          <div className="w-full max-w-md mx-auto lg:mx-0 lg:ml-auto">
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl px-4 py-3 text-center">
                ...
                <p className="text-xs text-[#666] font-medium mt-0.5">Languages</p>
              </div>
              <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl px-4 py-3 text-center">
                <p className="text-2xl font-black text-green-400">&lt;200ms</p>
                <p className="text-xs text-[#666] font-medium mt-0.5">Avg Execution</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="px-6 py-16 max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-[#b3b3b3] max-w-lg mx-auto">
            A professional-grade IDE experience, designed specifically for students and learners.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {FEATURES.map(({ icon: Icon, title, desc, color, bg }) => (
            <div key={title} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-5 hover:border-primary/30 transition-colors group">
      ...
      <section className="px-6 py-16 max-w-7xl mx-auto">
        <div className="relative bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-10 text-center overflow-hidden">
          ...
          <div className="relative">
            <h2 className="text-2xl md:text-3xl font-bold mb-3">Ready to start coding?</h2>
            <p className="text-[#b3b3b3] mb-7 max-w-md mx-auto">
              Sign in with your SkillDzire account to save projects, track execution history, and code from anywhere.
            </p>
